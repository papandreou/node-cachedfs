...
function CachedFs(options) {
    ...
    if (options.cache) {
        ...
        var lruCacheOptions = {
            length: CachedFs.estimateLengthOfMemoizedValue
        };
        for (var propertyName in options) {
            if (options.hasOwnProperty(propertyName) &&
                propertyName !== 'fs' &&
                propertyName !== 'context' &&
                propertyName !== 'cacheKeyPrefix' &&
                propertyName !== 'debug' &&
                propertyName !== 'skipUnimplemented') {
    ...
    ['stat', 'lstat', 'readlink', 'realpath', 'readdir', 'readFile', 'exists'].forEach(function (asyncFunctionName) {
        ...
        if (syncFunctionName in fs) {
            ...
            that[syncFunctionName] = function () { // ...
                ...
                if (!resultCallbackParams && typeof arguments[1] === 'string') {
                    var path = arguments[0],
                        encoding = arguments[1],
                        encodingLessResultCallbackParams = memoizedAsyncFunction.peek.apply(memoizedAsyncFunction, [path]);
                    if (encodingLessResultCallbackParams) {
                        var err = encodingLessResultCallbackParams[0],
                            result = encodingLessResultCallbackParams[1];
                        resultCallbackParams = [err];
                        if (Buffer.isBuffer(result)) {
                            resultCallbackParams.push(result.toString(encoding));
                        }
                    }
                }
