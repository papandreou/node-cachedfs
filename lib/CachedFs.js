...
function CachedFs(fs, options) {
    ...
    var cache = that.cache = options.cache || new LRUCache(options),
        cacheKeyPrefix;
    ...
    ['stat', 'lstat', 'fstat', 'readlink', 'realpath', 'readdir', 'readFile', 'exists'].forEach(function (asyncFunctionName) {
        var memoizedAsyncFunction,
            memoizerOptions = {argumentsStringifier: argumentsStringifier, cacheKeyPrefix: getCacheKey(asyncFunctionName), cache: cache};
